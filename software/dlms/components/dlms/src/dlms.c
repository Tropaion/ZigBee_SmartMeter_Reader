...
/* ===== DLMS Layer ===== */
static uint8_t gue_key[GUE_KEY_LENGTH] = {0x52, 0xFC, 0xF7, 0x0B, 0xC3, 0x94, 0x9B, 0x61, 0x3E, 0xC9, 0xE7, 0xB1, 0x5F, 0xAD, 0x55, 0xEF};

static esp_err_t apdu_decrypt(uint8_t* user_data, uint16_t user_data_size)
{
    /* Encrypted data buffer */
    uint8_t encrypted_data[DATA_BUFFER_SIZE];
    uint16_t encrypted_data_size = 0;

    /* Frame counter for decryption */
    uint8_t frame_counter[DLMS_FRAME_COUNTER_SIZE];

    /* ===== GET RELEVANT DATA ===== */
    /* This part can be different for other smartmeters */

    /* Get frame counter */
    memcpy(&frame_counter[0], &user_data[DLMS_FRAME_COUNTER_OFFSET], DLMS_FRAME_COUNTER_SIZE);

    /* Copy first frame, to get relevant data to decrypt */
    /* Calculate size of first frame */
    if(user_data_size < DLMS_MAX_SIZE)
    {
        encrypted_data_size = user_data_size - DLMS_FIRST_FRAME_OFFSET;
    }
    else
    {
        encrypted_data_size = DLMS_MAX_SIZE - DLMS_FIRST_FRAME_OFFSET;
    }
    /* Copy first frame */
    memcpy(&encrypted_data[0], &user_data[DLMS_FIRST_FRAME_OFFSET], encrypted_data_size);

    /* Check if data consists of multiple frames */
    for(int i = DLMS_MAX_SIZE; user_data_size > i; i += DLMS_MAX_SIZE)
    {
        ...
        {
            frame_size = DLMS_MAX_SIZE - DLMS_SECONDARY_FRAME_OFFSET;
        }
        else
        {
            frame_size = user_data_size - i - DLMS_SECONDARY_FRAME_OFFSET;
        }
        /* Copy frame */
        memcpy(&encrypted_data[encrypted_data_size], &user_data[i + DLMS_SECONDARY_FRAME_OFFSET], frame_size);
...
esp_err_t dlms_init()
{
    ...
    apdu_decrypt(&user_data[0], user_data_size);
